package com.rm13.spring.ioc.config;

import com.rm13.spring.ioc.condition.*;
import com.rm13.spring.ioc.entity.Color;
import com.rm13.spring.ioc.entity.ColorFactoryBean;
import com.rm13.spring.ioc.entity.Person;
import com.rm13.spring.ioc.entity.Red;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.config.ConfigurableBeanFactory;
import org.springframework.context.annotation.*;

/**
 *
 * @Conditional 类中组件统一设置。满足当前条件，这个类中配置的所有bean注册才能生效；
 * @Import 导入组件，id默认是组件的全类名
 */
@Slf4j
@Configuration
@Import({Color.class, Red.class, MyImportSelector.class, MyImportBeanDefinitionRegistrar.class})
public class MainConfigOfBeanDefinition02 {


    /**
     * @see ConfigurableBeanFactory#SCOPE_PROTOTYPE
     * @see ConfigurableBeanFactory#SCOPE_SINGLETON
     * @see org.springframework.web.context.WebApplicationContext#SCOPE_REQUEST  request
     * @see org.springframework.web.context.WebApplicationContext#SCOPE_SESSION	 sesssion
     * @return
     * @Scope:调整作用域
     * prototype：多实例的：ioc容器启动并不会去调用方法创建对象放在容器中。
     * 					每次获取的时候才会调用方法创建对象；
     * singleton：单实例的（默认值）：ioc容器启动会调用方法创建对象放到ioc容器中。
     * 			以后每次获取就是直接从容器（map.get()）中拿，
     * request：同一次请求创建一个实例
     * session：同一个session创建一个实例
     *
     * 懒加载：
     * 		单实例bean：默认在容器启动的时候创建对象；
     * 		懒加载：容器启动不创建对象。第一次使用(获取)Bean创建对象，并初始化；
     *
     */

    @Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
    @Lazy
    @Bean("singleton")
    public Person person(){
        log.info("@Lazy..singleton..Person....");
        return new Person("Lazy", 25);
    }

    @Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
    @Bean("prototype")
    public Person person01(){
        log.info("@Scope..prototype..Person....");
        return new Person("prototype", 25);
    }


    /**
     * @Conditional({Condition}) ： 按照一定的条件进行判断，满足条件给容器中注册bean
     * 如果系统是windows，给容器中注册("bill")
     * 如果是linux系统，给容器中注册("linus")
     * 如果是mac系统，给容器中注册("Steve")
     * @return
     */
    @Conditional(WindowsCondition.class)
    @Bean("bill")
    public Person person02(){
        log.info("WindowsCondition..Bill Gates..Person....");
        return new Person("Bill Gates", 25);
    }

    @Conditional(LinuxCondition.class)
    @Bean("linus")
    public Person person03(){
        log.info("LinuxCondition..linus..Person....");
        return new Person("linus", 25);
    }

    @Conditional(MacCondition.class)
    @Bean("Steve")
    public Person person04(){
        log.info("MacCondition..Steve jobs..Person....");
        return new Person("Steve jobs", 25);
    }


    /**
     * 给容器中注册组件:
     * 1）、包扫描+组件标注注解（@Controller/@Service/@Repository/@Component）[自己写的类]
     * 2）、@Bean[导入的第三方包里面的组件]
     * 3）、@Import[快速给容器中导入一个组件]
     * 		1）、@Import(要导入到容器中的组件)；容器中就会自动注册这个组件，id默认是全类名
     *  	2）、ImportSelector:返回需要导入的组件的全类名数组；
     *      3）、ImportBeanDefinitionRegistrar:手动注册bean到容器中
     * 4）、@ImportResource [快速给容器中导入一个组件]
     *      @ImportResource({"classpath:spring/applicationContext-*.xml"})
     *
     * 5）、使用Spring提供的 FactoryBean（工厂Bean）;
     *  	1）、默认获取到的是工厂bean调用getObject创建的对象
     *   	2）、要获取工厂Bean本身，我们需要给id前面加一个&, 如：(&colorFactoryBean)
     * @return
     */
    @Bean
    public ColorFactoryBean colorFactoryBean(){
        return new ColorFactoryBean();
    }


}
